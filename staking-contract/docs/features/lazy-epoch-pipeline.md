...
| Deferred subscription stake decrease | Queues surplus unstake only after the validator settlement preamble has run. Due updates that include a stake decrease are routed through the same settlement pipeline before calling the internal unstake path. The off-chain staking routine is responsible for periodically touching due subscriptions after `apply_ns` by using public contract flows, so storage commits and unstake queueing do not depend only on the subscriber’s next write. |
