...
3. **Locks** — Users **`lock`** / **`lock`**: mint internal shares, queue stake, enforce price vs locked amount × duration (`lock.rs`, `utils.rs`).
4. **Subscriptions** — One subscription per `(account, product)`; cancel, upgrade, downgrade, getters (`subscriptions.rs`); **`lock`** and renewal prorate hook (`lock.rs`); unlock path (`unlock.rs`).
