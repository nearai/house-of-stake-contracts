...
- **Per NEAR `epoch_height`**: at most **one** successful pool **`deposit_and_stake`** **or** **`unstake`** (or inline net-zero clear); updates **`last_settlement_epoch`**. Fresh-epoch no-pending and unstake-waiting settlement decisions also update **`last_settlement_epoch`** after the pool account check succeeds.
