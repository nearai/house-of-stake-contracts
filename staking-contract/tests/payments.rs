...
#[test]
fn pay_indexes_purchases_by_product_without_product_vec_growth() {
    let mut c = deploy();
    let (product_id, price_id) = setup_catalog_near_oneoff(&mut c);
    register_buyer(&mut c);
    ...
    assert_eq!(
        c.purchase_count_by_product.get(&product_id).copied(),
        Some(3)
    );
    assert_eq!(
        c.purchase_count_by_account.get(&acct(BUYER)).copied(),
        Some(3)
    );
    assert_eq!(
        c.purchases_by_product
            .get(&(product_id.clone(), 0))
            .expect("first product purchase"),
        &purchase_ids[0]
    );
    assert_eq!(
        c.purchases_by_product
            .get(&(product_id.clone(), 2))
            .expect("third product purchase"),
        &purchase_ids[2]
    );
    assert_eq!(
