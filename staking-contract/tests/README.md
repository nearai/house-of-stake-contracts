...
[`sandbox_mock_pool.rs`](sandbox_mock_pool.rs) deploys built **`staking_contract.wasm`** and **`mock_staking_pool_contract.wasm`** and exercises real cross-contract calls (catalog, `lock_for_product`, pool views, `epoch_settle`, `withdraw(validator_id)`, etc.). Helpers live in [`mock_pool/mod.rs`](mock_pool/mod.rs). Requires a **near-workspaces**–supported host (linux-x86 or darwin-arm) to run.
